    ...
    return (
        <TouchableOpacity style={[Styles.container, props.itemStyle, props.IsLeft ? Styles.margRight : null]} activeOpacity={1}>
            <View style={Styles.imgContainer}>
                <View style={{ flexDirection: 'row', width: '100%' }}>
                    <View style={{ flex: 0.5, alignItems: 'flex-start' }}>
                        <IconButton icon="heart" size={25} color={!itemData.IsFav ? Colors.light : Colors.primary} onPress={updateItem} style={Styles.iconContainer} />
                    </View>
                    <View style={{ flex: 0.5, alignItems: 'flex-end' }}>
                        {itemData.discount !== '0%' && <Text style={Styles.discountStyle}>{itemData.discount}</Text>}
                    </View>
                </View>
            ...
            <View style={Styles.itemDetailsContainer}>
                <Text ellipsizeMode='tail' numberOfLines={1} style={Styles.title}>{itemData.name}</Text>
                <StarRating
                    disabled={true}
                    emptyStar={'star'}
                    fullStar={'star'}
                ...
                />
                <Text ellipsizeMode='tail' numberOfLines={1} style={Styles.price}>{itemData.price}</Text>
            </View>
